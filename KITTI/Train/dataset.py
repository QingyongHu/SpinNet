...
class KITTIDataset(Data.Dataset):
    ...
    def initial(self):
        ...
        for i in range(len(self.patches)):
            ...
            pose = np.concatenate([poses[ind][:3, :3].reshape(9), poses[ind][:3, 3]]).reshape(2, 6).astype(np.float32)
        ...
        if self.drop_last:
            if self.current_patches_num <= (len(self.patches) % self.batch_size):  # reach the end of training file
                ...
                if self.num_file < len(self.training_data_files):
                    ...
                    for i in range(len(self.patches)):
                        ind = int(np.floor(i / self.per_num_patch))
                        pose = np.concatenate([poses[ind][:3, :3].reshape(9), poses[ind][:3, 3]]).reshape(2, 6).astype(
                            np.float32)
                        self.patches[i] = np.concatenate([pose, self.patches[i]])
                    self.cur_pose_ind = next_pose_ind
...
if __name__ == "__main__":
    data_root = "../../data/KITTI_patches/"
    batch_size = 48
    epoch = 1
    train_dataset = KITTIDataset(root=data_root, split='train', batch_size=batch_size, shuffle=True, drop_last=True)
